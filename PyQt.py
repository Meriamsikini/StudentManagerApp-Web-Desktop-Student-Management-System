...
class DataManager:
    def __init__(self, file_path=None):
        # Utilise le chemin absolu vers le fichier CSV dans le même dossier que ce script
        if file_path is None:
            base_dir = os.path.dirname(os.path.abspath(__file__))
            file_path = os.path.join(base_dir, "etudiants.csv")
    ...
    def load_data(self):
        try:
            ...
            print(f"[INFO] Fichier CSV introuvable. Création de : {self.file_path}")
    ...
    # Correction du nom de colonne et conversion en float
      self.df["Note"] = pd.to_numeric(self.df["Note"], errors="coerce")
...
class StudentManagementApp(QMainWindow):
    ...
    def initUI(self):
        ...
        base_dir = os.path.dirname(os.path.abspath(__file__))
        icon_clear = QIcon(os.path.join(base_dir, "static", "icons8-clear-50.png"))
        clear_action = QAction(icon_clear, "", self)
        ...
        icon_back = QIcon(os.path.join(base_dir, "static", "icons8-back-50.png"))
        btn_action = QAction(icon_back, "", self)
        self.btn_back = QPushButton(icon_back, "", self)
